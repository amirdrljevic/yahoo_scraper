...
begin
  ...
  #create a resulting hash, to be populated later
  output = {}  
  ...
  #Find Year founded (This is the only place I was able to find this information, so I had to improvise)
  ...
  #If json output file exists, delete it
  File.delete("result_#{ARGV[0]}.json") if File.exist?("result_#{ARGV[0]}.json")

  #Save the hash output in a json file
  File.open("result_#{ARGV[0]}.json","w") do |f|
    f.write(output.to_json)
  end

  #Save the hash outout into a sqlite3 db
  #If the output db file exists, delete it
